...
def Conv2D(
    ...
    with tf.name_scope(name) as scope:
        ...
        if mask_type is not None:
            ...
            mask = np.ones(
                (filter_size, filter_size, input_dim, output_dim), dtype="float32"
            )
        ...
        if _weights_stdev is not None:
            filter_values = uniform(
                _weights_stdev, (filter_size, filter_size, input_dim, output_dim)
            )
        else:
            filter_values = uniform(
                filters_stdev, (filter_size, filter_size, input_dim, output_dim)
            )
        ...
        if weightnorm:
            ...
            with tf.name_scope("weightnorm") as scope:
                norms = tf.sqrt(
                    tf.reduce_sum(tf.square(filters), reduction_indices=[0, 1, 2])
                )
        ...
        result = tf.nn.conv2d(
            ...
            padding= [[0, 0], [0, 0], [padding, padding+1], [padding, padding+1]] ,# "SAME", # 
            data_format="NCHW",
#             dilations=[1, 1, 1, 1],
        )
