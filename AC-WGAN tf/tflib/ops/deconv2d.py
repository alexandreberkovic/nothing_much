    ...
    with tf.name_scope(name) as scope:
        ...
        if _weights_stdev is not None:
            filter_values = uniform(
                _weights_stdev, (filter_size, filter_size, output_dim, input_dim)
            )
        else:
            filter_values = uniform(
                filters_stdev, (filter_size, filter_size, output_dim, input_dim)
            )
        ...
        if weightnorm:
            ...
            with tf.name_scope("weightnorm") as scope:
                norms = tf.sqrt(
                    tf.reduce_sum(tf.square(filters), reduction_indices=[0, 1, 3])
                )
