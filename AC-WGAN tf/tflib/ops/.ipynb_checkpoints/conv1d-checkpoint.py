...
def Conv1D(
    ...
    with tf.name_scope(name) as scope:
        ...
        if mask_type is not None:
            ...
            mask = np.ones((filter_size, input_dim, output_dim), dtype="float32")
            ...
            for i in xrange(mask_n_channels):
                for j in xrange(mask_n_channels):
                    if (mask_type == "a" and i >= j) or (mask_type == "b" and i > j):
                        mask[center, i::mask_n_channels, j::mask_n_channels] = 0.0
        ...
        if weightnorm:
            ...
            with tf.name_scope("weightnorm") as scope:
                norms = tf.sqrt(
                    tf.reduce_sum(tf.square(filters), reduction_indices=[0, 1])
                )
...
#         )
        ...
        if biases:
            _biases = lib.param(
                name + ".Biases", np.zeros([output_dim], dtype="float32")
            )

            # result = result + _biases

            result = tf.expand_dims(result, 3)
            result = tf.nn.bias_add(result, _biases, data_format="NCHW") #"NCHW"
