    ...
    if ((axes == [0, 2, 3]) or (axes == [0, 2])) and fused == True:
        ...
        def _fused_batch_norm_training():
            return tf.nn.fused_batch_norm(
                inputs, scale, offset, epsilon=1e-5, data_format="NCHW" #"NCHW"
            )
        ...
        def _fused_batch_norm_inference():
            ...
            mean = ((1.0 / batch_size) * mean) + (
                ((batch_size - 1.0) / batch_size) * moving_mean
            )[None, :, None, None]
            var = ((1.0 / batch_size) * var) + (
                ((batch_size - 1.0) / batch_size) * moving_variance
            )[None, :, None, None]
            return (
